  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-white/95 backdrop-blur-md shadow-lg shadow-purple-900/5 py-3 border-b border-purple-100 opacity-100 translate-y-0 text-slate-900"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          >
            <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden border-2 border-purple-500/80 shadow-md shadow-purple-500/20 group-hover:scale-105 transition-transform duration-200 shrink-0">
              <Image
                src="/logo.png"
          ...
          <nav className="hidden lg:flex items-center gap-1 bg-purple-50/90 p-1.5 rounded-full border border-purple-100/80 shadow-2xs">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-4 py-1.5 text-xs font-bold text-slate-700 hover:text-purple-700 hover:bg-white rounded-full transition-all duration-200"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden sm:flex items-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-600 text-white text-xs font-extrabold shadow-md shadow-emerald-500/20 hover:shadow-lg hover:shadow-emerald-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200"
            >
